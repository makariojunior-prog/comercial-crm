...
export default function FinalizarConciliacaoModal({ item, existing, onClose, onSaved }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-end sm:items-center justify-center">
      <div className="w-full sm:max-w-2xl bg-white dark:bg-slate-800 rounded-t-2xl sm:rounded-xl shadow-lg flex flex-col max-h-[90vh]">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-white dark:bg-slate-800">
          ...
          {/* Dados do Pedido */}
          <div className="space-y-2 bg-slate-50 dark:bg-slate-700 p-3 rounded border border-slate-200 dark:border-slate-600">
            <div className="text-sm font-semibold text-slate-700 dark:text-slate-200">Dados do Pedido</div>
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div>
                <div className="text-slate-600 dark:text-slate-400">Empresa</div>
          ...
          <div className="space-y-2">
            ...
            {/* Resumo */}
            <div className="space-y-1 bg-blue-50 dark:bg-blue-950/40 p-3 rounded border border-blue-200 dark:border-blue-800 mt-3">
              ...
              {divergencia !== 0 && (
                <div className={`flex justify-between text-sm font-semibold ${divergencia > 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
          ...
          {/* Tipo de Ocorrência */}
          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-700 dark:text-slate-200">Tipo de Ocorrência</label>
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              className="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 rounded focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm resize-none"
              rows={3}
            />
          </div>
        </div>
        ...
        {/* Footer */}
        <div className="flex gap-2 border-t border-gray-200 dark:border-slate-700 p-4 bg-slate-50 dark:bg-slate-750">
          <button
            onClick={onClose}
            disabled={saving}
            className="flex-1 px-4 py-2 border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-slate-300 rounded hover:bg-gray-50 dark:hover:bg-slate-700 disabled:opacity-50"
          >
            Cancelar
          </button>
          <button
