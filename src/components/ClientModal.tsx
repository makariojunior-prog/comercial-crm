...
import { X, User, Phone, MapPin, Building2, MessageSquare, Save, AlertCircle, CheckCircle2, AlertTriangle, UserX, Briefcase, Package, Wrench, MessageCircle, Star, ShieldCheck } from 'lucide-react'
      ...
      <div className="relative bg-white dark:bg-slate-800 w-full max-w-2xl rounded-t-2xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[92vh]">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-6">
          ...
          <section className="space-y-4">
            ...
            {/* Carimbo de cadastro — apenas em modo edição */}
            {client && (
              <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-700/60 border border-slate-200 dark:border-slate-600 select-none">
                <ShieldCheck size={14} className="text-slate-400 shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-[10px] font-black uppercase text-slate-400 tracking-wide">Carimbo do sistema</p>
                  <p className="text-xs font-semibold text-slate-600 dark:text-slate-300">
                    Cadastrado em{' '}
                    {(() => {
                      try {
                        return format(parseISO(client.created_at), "dd 'de' MMMM 'de' yyyy 'às' HH:mm", { locale: ptBR })
                      } catch {
                ...
                <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-slate-200 dark:bg-slate-600 text-slate-400 uppercase shrink-0">
                  Imutável
                </span>
              </div>
            )}
